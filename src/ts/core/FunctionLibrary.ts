...
export function setupMeshProperties(child: any): void
{
	...
	if (child.material.map !== null)
	{
		...
		// Fence Art
		if ((child.name === 'Plane.018_1') || (child.name === 'Plane.072_1')) {
			mat.alphaMap = child.material.alphaMap;
			mat.transparent = true;
			mat.side = THREE.DoubleSide;
		}

		// Decal Art
		if (child.name.includes('TrackDecals') || child.name === 'Plane034' || child.name === 'Plane032') {
		...
		// Marking Art
		if (child.name.includes('Markings') || child.name.includes('BézierCurve.008')) {
			// alpha
			child.castShadow = false;
			mat.alphaMap = child.material.map;
			mat.transparent = true;
			mat.side = THREE.DoubleSide;
			if (child.name.includes('BézierCurve.008')) {
				child.position.y += 0.01;
			}

			// color
			if (child.name === 'Markings006') {
				mat.color = new THREE.Color('rgba(120, 120, 120, 1)');
			}
		}
