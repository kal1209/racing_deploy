...
import { pressKeyboard, moveCharacters } from '../socket/socket';
...
export class InputManager implements IUpdatable
{
	...
	public currentState: string = 'key';
	...
	{	
		if (this.inputReceiver !== undefined)
		{
			...
			if (this.world.mode === 'hub') {
				if (World.enablePressButton) this.inputReceiver.handleKeyboardEvent(event, event.code, true);
			} else {
				if (this.currentState !== event.code) {
					pressKeyboard(event, event.code, this.world, true);
					this.currentState = event.code
				}
			}
		}
	}
	...
	public onKeyUp(event: KeyboardEvent): void
	{
		if (this.inputReceiver !== undefined)
		{
			this.currentState = 'keyUp';
			if (this.world.mode === 'hub') {
				if (World.enablePressButton) this.inputReceiver.handleKeyboardEvent(event, event.code, false);
			} else {
				if (this.currentState !== event.code) {
					pressKeyboard(event, event.code, this.world, false);
					// this.currentState = event.code
				}
			}
		}
