...
import { updateVehicles, mySid, timeStamp, changeStatus,playersList_ } from '../socket/socket';
import { enableCharacterUpdate, enableVehicleUpdate, realtimeDetect } from '../world/World';
...
{	
	...
	public start: any = performance.now();
	...
	{		
		...
		if (this.physicsEnabled)
		{
			if (mySid === this.sid) {
				...
				if (this.detectPosition) {
					// Interpolation
					if (enableCharacterUpdate && realtimeDetect) {
						if (this.detectionStart) {
							let newPosition = this.interpolation(this.position, this.detectPosition, timeStamp / 2000);
	...
	public interpolation(vector1: THREE.Vector3, vector2: THREE.Vector3, duration: any) {
	...
	public startControllingVehicle(vehicle: IControllable, seat: VehicleSeat): void
	{
		if (this.controlledObject !== vehicle)
		{
			...
			this.controlledObject.sid = this.sid;

			this.world.players.forEach((player, index) => {
				if (this.sid === player.mySid) {
					if (index !== 0) {
						player.bubble.worldObject = vehicle;
					}
				}
			})
			
			World.enablePressButton = true;
			if (enableVehicleUpdate && realtimeDetect) {
				changeStatus('vehicle');
				updateVehicles(this.world, this.sid);
				if (this.sid === mySid) this.world.cameraOperator.followMode = true;
			}
		}
	}
