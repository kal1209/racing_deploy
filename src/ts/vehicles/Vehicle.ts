...
import { World, realtimeDetect } from '../world/World';
...
import { mySid, timeStamp } from '../socket/socket';
import { enableVehicleUpdate } from '../world/World';
...
export abstract class Vehicle extends THREE.Object3D implements IWorldEntity
{
	...
	public detectPosition: THREE.Vector3 = new THREE.Vector3();
	public detectQuaternion: CANNON.Quaternion = new CANNON.Quaternion();
	public detectionStart: boolean = false;
	public modelContainer: THREE.Group;
	private detectFlag: boolean = false;
	...
	{	
		if ( enableVehicleUpdate && realtimeDetect && this.sid !== undefined && this.sid !== mySid ) {
			if (this.detectionStart) {
				if (this.position.distanceTo(this.detectPosition) < 10) this.detectFlag = true;
				if (this.detectFlag) {
					let newPosition = this.interpolationPosition(this.position, this.detectPosition, timeStamp / 1000);
					this.setPosition(newPosition.x, newPosition.y, newPosition.z);
					this.position.set(newPosition.x, newPosition.y, newPosition.z);

					this.collision.interpolatedPosition.x = this.position.x;
					this.collision.interpolatedPosition.y = this.position.y;
					this.collision.interpolatedPosition.z = this.position.z;

					let threeQuater = new THREE.Quaternion();
					threeQuater.set(
						this.detectQuaternion.x,
						this.detectQuaternion.y,
						this.detectQuaternion.z,
						this.detectQuaternion.w
					);

					let newQuaternion = this.interpolationQuaternion(this.quaternion, threeQuater, timeStamp / 1000);
					this.quaternion.copy(newQuaternion);
					this.collision.quaternion.x = this.quaternion.x;
					this.collision.quaternion.y = this.quaternion.y;
					this.collision.quaternion.z = this.quaternion.z;
					this.collision.quaternion.w = this.quaternion.w;
				}
			}
		} else {
			this.position.set(
	...
	public interpolationPosition(vector1: THREE.Vector3, vector2: THREE.Vector3, duration: any) {
		let result = new THREE.Vector3();
		let distance = new THREE.Vector3().copy(vector2).sub(vector1);
		let progress = Math.min(this.world.clock.getDelta() / duration, 1);
		if (progress === 0) result = vector2;
		else {
			result = new THREE.Vector3().copy(vector1).add(new THREE.Vector3().copy(distance).multiplyScalar(progress));
		}
		return result;
	}

	public interpolationQuaternion(quater1: THREE.Quaternion, quater2: THREE.Quaternion, duration: any) {
		let result = new THREE.Quaternion();
		let progress = Math.min(this.world.clock.getDelta() / duration, 1);
		if (progress === 0) result = quater2;
		result.x = progress * quater1.x + duration * quater2.x;
		result.y = progress * quater1.y + duration * quater2.y;
		result.z = progress * quater1.z + duration * quater2.z;
		result.w = progress * quater1.w + duration * quater2.w;
		result.normalize(); // Normalize the resulting quaternion
		return result;
	}
